...
from PySide6.QtGui import QBrush, QColor, QFontMetrics, QGuiApplication, QPainter, QPen
...
class SeeingNightPlannerWidget(QWidget):
    ...
    def paintEvent(self, event):  # noqa: N802 - Qt override
        ...
        for block_index, block in enumerate(self.blocks):
            ...
            moon_font.setPointSize(max(24, moon_old + 10))
            painter.setFont(moon_font)
            painter.setPen(QColor("#dbe8f5"))
            painter.drawText(QRectF(moon_x, y + 13, 44, 38), Qt.AlignCenter, moon_glyph)
            ...
            dark_rect = QRectF(left + 174, y + 66, max(135.0, info_w - 188.0), 24)
            dark_text = QFontMetrics(painter.font()).elidedText(
                f"Dark {dark_points} pts · Moon up {moon_points} pts",
                Qt.ElideRight,
                int(dark_rect.width()),
            )
            painter.drawText(dark_rect, Qt.AlignLeft | Qt.AlignVCenter, dark_text)
    ...
    def _build_24h_blocks(self, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        while cursor <= last_dt:
            ...
            if block_rows:
                ...
                display_dt = first_block_dt if first_block_dt.date() > cursor.date() else cursor
