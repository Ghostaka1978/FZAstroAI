...
Add-Content -Path $ValidationLog -Value "EXE: $ExePath"
Add-Content -Path $ValidationLog -Value "SHA256: $hash"
Add-Content -Path $ValidationLog -Value "SizeMB: $sizeMB"

$VersionFile = Join-Path $ProjectRoot "VERSION.txt"
if (Test-Path $VersionFile) {
    $version = (Get-Content $VersionFile -Raw).Trim()
    if ($version -match "^1\.0\.0$") { Add-Content -Path $ValidationLog -Value "VERSION.txt reports $version" }
    else { Write-Warning "VERSION.txt does not look like v1.0.0: $version" }
}
...
if (-not $SkipLaunch) {
    ...
    Add-Content -Path $ValidationLog -Value "Smoke test FZASTRO_APP_DIR: $SmokeAppDir"
    try {
