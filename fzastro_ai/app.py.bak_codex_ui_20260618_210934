...
from .web_companion.launcher import WebCompanionProcess, lan_web_url, local_web_url
...
class FZAstroAI(
    ...
    def __init__(self):
        ...
        self.web_companion_button.setAccessibleName("Manage Web Companion")
        self.web_companion_button.setMenu(self.build_web_companion_menu())
        ...
        self.web_companion_auto_start_checkbox.setChecked(
            bool(self.web_companion_settings.get("auto_start_desktop"))
        )
        self.web_companion_auto_start_checkbox.stateChanged.connect(
            self.set_web_companion_auto_start
        )
        ...
        web_layout.addWidget(self.web_companion_start_button)

        self.web_companion_open_button = QPushButton("Open Web Companion")
        self.web_companion_open_button.clicked.connect(self.open_web_companion)
        web_layout.addWidget(self.web_companion_open_button)

        self.web_companion_lan_button = QPushButton("Start LAN / iPad Mode...")
        self.web_companion_lan_button.clicked.connect(
            self.start_web_companion_lan_prompt
        )
        web_layout.addWidget(self.web_companion_lan_button)

        web_action_row = QHBoxLayout()
        web_action_row.setContentsMargins(0, 0, 0, 0)
        web_action_row.setSpacing(7)

        self.web_companion_copy_button = QPushButton("Copy Current URL")
        self.web_companion_copy_button.clicked.connect(
            self.copy_web_companion_current_url
        )
        web_action_row.addWidget(self.web_companion_copy_button, 1)

        self.web_companion_stop_button = QPushButton("Stop")
        self.web_companion_stop_button.setObjectName("dangerActionButton")
        self.web_companion_stop_button.clicked.connect(
            self.stop_web_companion_background
        )
        web_action_row.addWidget(self.web_companion_stop_button, 1)

        web_layout.addLayout(web_action_row)
        sidebar_layout.addWidget(web_card)
    ...
    def set_web_companion_auto_start(self, state):
        enabled = int(state) != 0
    ...
    def build_web_companion_menu(self):
        ...
        open_action = QAction("Open Web Companion", self)
        ...
        start_lan_action = QAction("Start LAN server...", self)
        ...
        copy_local_action = QAction("Copy current URL", self)
        copy_local_action.triggered.connect(self.copy_web_companion_current_url)
        menu.addAction(copy_local_action)

        copy_lan_action = QAction("Copy LAN/iPad URL", self)
        copy_lan_action.triggered.connect(self.copy_web_companion_lan_url)
        menu.addAction(copy_lan_action)

        status_action = QAction("Show status", self)
        status_action.triggered.connect(self.show_web_companion_status)
    ...
    def start_web_companion_lan_prompt(self):
        token, accepted = QInputDialog.getText(
            self,
            "Start Web Companion LAN",
            "Token required for iPad/Mac LAN access:",
            QLineEdit.Password,
            "fzastro",
        )
        ...
        if not token:
            QMessageBox.warning(
                self,
                "Web Companion",
                "LAN mode requires a token. The server was not started.",
            )
            return
        ...
        if status.running:
            QApplication.clipboard().setText(status.url)
            QMessageBox.information(
                self,
                "Web Companion LAN",
                "Web Companion is available on the same network.\n\n"
                f"URL copied to clipboard: {status.url}\n"
                f"Token: {token}\n\n"
                "Open this URL from your iPad/Mac/phone while connected to the same network.",
            )
        else:
            QMessageBox.warning(self, "Web Companion", status.message)
    ...
    def open_web_companion(self):
        ...
        QDesktopServices.openUrl(
            QUrl(status.url if status.running else local_web_url())
        )

    def copy_web_companion_current_url(self):
        status = self.web_companion.status()
        url = status.url if status.running else local_web_url()
        QApplication.clipboard().setText(url)
        self.update_web_companion_button(status)
        self.status_label.setText(f"Copied Web Companion URL: {url}")

    def copy_web_companion_lan_url(self):
        url = lan_web_url()
        QApplication.clipboard().setText(url)
        self.update_web_companion_button(self.web_companion.status())
        self.status_label.setText(f"Copied LAN/iPad Web Companion URL: {url}")

    def show_web_companion_status(self):
        status = self.web_companion.status()
        self.update_web_companion_button(status)
        detail = (
            f"Running: {status.running}\n"
            f"Started by desktop app: {status.owned}\n"
            f"URL: {status.url}\n"
            f"LAN mode: {status.lan}\n"
            f"PID: {status.pid or 'n/a'}\n\n"
            f"{status.message}"
        )
        QMessageBox.information(self, "Web Companion Status", detail)

    def update_web_companion_button(self, status=None):
        if status is None:
            status = self.web_companion.status()
    ...
    def update_web_companion_sidebar(self, status=None):
        if status is None:
            status = self.web_companion.status()

        label = getattr(self, "web_companion_status_label", None)
        if label is not None:
            owner = "desktop app" if status.owned else "manual/external"
            state = "Running" if status.running else "Stopped"
            extra_url = (
                lan_web_url(self.web_companion.port)
                if status.lan
                else local_web_url(self.web_companion.port)
            )
            label.setText(
                f"Status: {state}\n"
