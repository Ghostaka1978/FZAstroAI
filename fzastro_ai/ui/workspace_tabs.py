...
def _make_workspace_tab_close_icon(color: str = "#aeb8c4") -> QIcon:
    ...
    painter.setPen(
        QPen(QColor(color), 1.5, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap)
    )
    painter.drawLine(3, 3, 7, 7)
    painter.drawLine(7, 3, 3, 7)
    painter.end()
    return QIcon(pixmap)
...
class WorkspaceTabsMixin:
    ...
    def close_workspace_tab(self, index: int):
        ...
        try:
            ...
            log_exception("WorkspaceTabsMixin.close_workspace_tab", exc)
        finally:
            self._workspace_tab_closing.discard(widget_id)
    ...
    def _install_workspace_tab_close_button(self, index: int, widget: QWidget):
        ...
        button.setAccessibleName("Close workspace tab")
        button.setIcon(_make_workspace_tab_close_icon("#8b949e"))
