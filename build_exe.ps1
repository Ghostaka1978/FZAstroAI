...
function Install-PlaywrightChromiumBrowser {
    ...
    if (Test-PlaywrightChromiumPayload -BrowsersDir $LocalBrowsersDir) {
        Add-Content -Path $BuildLog -Value "Playwright package-local Chromium payload found: $LocalBrowsersDir"
    ...
    if (Copy-PlaywrightCacheToLocalBrowsers -SourceDir $CacheBrowsersDir -DestinationDir $LocalBrowsersDir) {
        Add-Content -Path $BuildLog -Value "Copied Playwright browser cache from $CacheBrowsersDir to package-local $LocalBrowsersDir."
    }
...
if (-not $SkipDependencyInstall) {
    ...
    New-Item -ItemType Directory -Force -Path $LogDir | Out-Null
    Add-Content -Path $BuildLog -Value "Dependency install skipped."
}
...
if (-not $SkipPlaywrightBrowserInstall) {
    ...
    Add-Content -Path $BuildLog -Value "Playwright Chromium browser install skipped."
}
