...
def _extract_python_symbols(path: Path, max_symbols: int = 80) -> tuple[tuple[str, ...], tuple[str, ...]]:
    try:
    ...
    return tuple(dict.fromkeys(symbols[:max_symbols])), tuple(dict.fromkeys(imports[:max_symbols]))
