...
ROLE_HINTS = {
    "ui": {"button", "dialog", "label", "layout", "screen", "tab", "ui", "widget", "window"},
    ...
    "dev_agent": {"code", "developer", "diff", "patch", "project", "scanner", "test runner"},
...
def _normalize_terms(text: str) -> tuple[str, ...]:
    ...
    for match in TOKEN_RE.finditer(text.lower()):
        ...
        if len(term) < 2:
            continue
        if term in {"the", "and", "for", "with", "this", "that", "from", "into", "please"}:
            continue
        terms.append(term)
...
def classify_dev_task(request: str) -> DevTask:
    ...
    role_hints = tuple(
        role for role, _score in sorted(role_scores.items(), key=lambda item: (-item[1], item[0]))
    )

    confidence = min(1.0, 0.25 + (scores.get(mode, 0) * 0.12) + (len(role_hints) * 0.08))
