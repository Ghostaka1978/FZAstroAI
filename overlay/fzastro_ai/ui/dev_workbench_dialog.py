...
class DevWorkbenchDialog(QWidget):
    ...
    def browse_root(self):
        selected = QFileDialog.getExistingDirectory(self, "Choose FZAstro AI project root", str(self.project_root))
        if selected:
    ...
    def build_context_plan(self):
        ...
        if not request:
            QMessageBox.information(self, "Developer request needed", "Enter a coding request first.")
            return
        try:
